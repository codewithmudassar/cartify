  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        setActiveImg(data.single.images[0]);
        setTotalPrice(data.single.price); // Set initial total price
      } catch (error) {
      ...
      <div className="lg:flex md:flex mb-5 gap-4 globalShadow rounded-xl pb-2">
        <div className="md:w-4/12">
          <div className="flex flex-col items-center mt-7 mb-5 mx-5">
            <div className="flex justify-center object-contain p-6 border w-80 border-gray-30 mb-5 shadow-inner shadow-blue-400">
              <img
                src={activeImg}
                alt="Active item"
            ...
            <div className="w-full max-w-md p-2 shadow-inner shadow-blue-300">
              <Slider {...settings}>
                {product.images.map((v, i) => (
                  <div key={i} className="p-1 w-20 h-20  object-cover">
                    <img
                      src={v}
                      className="w-16 h-20 p-3 border border-blue-300  rounded-md cursor-pointer"
                      alt={`Carousel item ${i}`}
        ...
        <div className="mt-20 flex-1">
          <div className="mb-8 border border-blue-200 rounded-xl shadow-xl inline-block px-3 bg-blue-200 text-blue-500 ml-5 ">{product?.category?.title}</div>
          ...
          <div className="flex items-center mt-10 ml-10">
            <div className="flex items-center mt-">
              <p className="text-blue-500 text-xl mr-4">Quantity:</p>
            </div>
            <div className="flex">
              <button 
          ...
          <div className="mt-10 ml-10 flex gap-10">
            <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
              {/* <button className="border px-6 sm:px-14 py-2 sm:py-4 font-bold text-white bg-blue-400 hover:bg-blue-500 w-full sm:w-auto">
                Buy Now
              </button> */}
              <button
                type="button"
                ...
                toast
                className="border px-6 sm:px-14 py-2 sm:py-4 font-bold text-white rounded-xl bg-blue-400 hover:bg-blue-500 w-full sm:w-auto"
              >
                Add to Cart
              </button>
            </div>
